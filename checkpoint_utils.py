#!/usr/bin/env python3
"""
Checkpoint utilities for 2048-Zero

Helper functions for downloading checkpoints from various sources
"""
...
def download_checkpoint(url: str, output_dir: str = "checkpoints") -> Optional[str]:
    ...
    try:
        ...
        # Also try to download run data
        try:
            run_url = url.replace(".pth", "_run.json")
            run_path = os.path.join(output_dir, os.path.basename(run_url))
...
def get_r2_signed_url(bucket: str, key: str, access_key: str, secret_key: str, 
...
def handle_r2_url(r2_url: str, env_vars: Dict[str, str] = None) -> Optional[str]:
    ...
    try:
        ...
        # Generate signed URL
        return get_r2_signed_url(bucket, key, access_key, secret_key, endpoint_url)
    except Exception as e:
        print(f"Failed to handle R2 URL: {e}")
        return None

def download_checkpoint_from_r2(r2_url: str, output_dir: str = "checkpoints") -> Optional[str]:
    """
    Download checkpoint from R2 URL
    ...
    """
    # Get HTTP URL from R2 URL
    http_url = handle_r2_url(r2_url)
    if not http_url:
        return None
    
    # Download checkpoint using HTTP URL
    return download_checkpoint(http_url, output_dir)